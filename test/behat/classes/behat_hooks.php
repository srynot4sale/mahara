...
class behat_hooks extends behat_base {
    ...
    public static function before_suite($event) {
        ...
        require_once('behat_selectors.php');
        require_once('behat_context_helper.php');
        ...
        // Check if the test site is installed and configure
        if (behat_util::check_test_site() !== BEHAT_OK) {
            throw new Exception('Test site is not installed or configured');
        }

        if (!behat_util::is_test_server_running()) {
            throw new Exception('The test server ' . $cfg->behat_wwwroot . ' is not available, ensure you specified correct url and that the server is set up and started.');
        }

        if (!(behat_util::get_test_site_status_from_file() == TEST_SITE_ENABLED)) {
            throw new Exception('Test site is not enabled.');
        }

    }

    /**
     * Resets the test database and dataroot before each feature test.
     * and also registers named selectors for mahara
     *
     * @throws Exception
     * @BeforeFeature
     */
    public static function before_feature(FeatureEvent $event) {
        ...
        // Prepare the database and dataroot for the feature test
        //behat_util::prepare_test_site();
    ...
    public function before_scenario($event) {
        ...
        if (!self::$initprocessesfinished) {
            ...
            $this->find("xpath", "//head/child::title[contains(., '" . behat_util::TESTSITENAME . "')]", $notestsiteexception);
     ...
     * @see mahara\BehatExtension\Tester\maharaStepTester
     */
